...
class DictWrapper(object):
    def __init__(self, xml, rootkey=None):
        ...
        self._mydict = objectify.fromstring(xml) #utils.xml2dict().fromstring(remove_namespace(xml))
        self._response_dict = self._mydict

    @property
    def parsed(self):
        if self._rootkey:
            return self._response_dict[self._rootkey]
        else:
