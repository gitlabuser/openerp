...
class sale_shop(osv.osv):
    ...
    def import_listing(self, cr, uid, id, shop_id, product_id ,resultvals, context={}):
        ...
        return super(sale_shop, self).import_listing(cr,uid,id,shop_id,product_id,resultvals,context)
...
#        
#        logger.error('createdAfter %s',createdAfter)
#        logger.error('createdBefore %s',createdBefore)
...
#        if results:
#            for result in results:
#                print'PurchaseDate',result['PurchaseDate']
#                saleorderids = sale_order_obj.search(cr,uid,[('name','=',instance_obj.prefix + result['OrderId'] + instance_obj.suffix),('shop_id','=',instance_obj.id)])
#                if saleorderids:
#                    if sale_order_obj.browse(cr,uid,saleorderids[0]).state != 'draft':
#                        print 'Order Exist',result['OrderId']
#                        continue
#                print "===result['OrderId']======>",result['OrderId']
#                result_vals = amazon_api_obj.call(instance_obj.instance_id,'ListOrderItems',result['OrderId'])
#
#                for result_val in result_vals:
#                    print 'result_val',result_val
    ...
    def handleMissingAsins(self, cr, uid,ids,missed_resultvals,context=None):
        ...
        while (missed_resultvals):
            ...
            for results in resultvals:
                ...
                try:
#                    synch_id = results['stock_sync_id']
#                    del results['stock_sync_id']
...
#                print "--->",e
                ...
                if str(e).find('concurrent update') != -1:
                    cr.rollback()
                    time.sleep(20)
        self.write(cr,uid,ids,{'report_update': datetime.datetime.now(),'requested_report_id':False})
        return True
